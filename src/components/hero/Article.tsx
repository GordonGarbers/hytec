...
import { IHero, IHeroDetails } from "../../interfaces/interfaces";
...
export const Article: React.FC<IArticleProps> = ({
  ...
  //CACHE IMAGES  -  NAPRAVI CUSTOM HOOKS
  ...
  useEffect(() => {
    async function fetchImageAndCache() {
      ...
      // Provjeri postoji li zahtjev u kešu
      const cachedResponse = await cache.match(request);

      if (cachedResponse) {
        // Ako postoji, postavi imageUrl na URL objekt koji prikazuje keširanu sliku
        const blob = await cachedResponse.blob();
        setImageUrl(URL.createObjectURL(blob));
      } else {
        // Ako ne postoji, dohvati sliku i spremi je u keš
      ...
      <div className="w-100 overflow-hidden article-right position-relative">
        <HeroCircularProgress remap={remap} />
        {
          ...
          <img
            ...
            src={isLoaded ? imageUrl : getImageRatio(1920, 1080)}
